...
We propose an interpretability pipeline that combines (i) sparse feature discovery (via sparse autoencoders), (ii) cross-layer transcoders to map internal activations into a consistent feature space across layers, and (iii) logic-based validators inspired by Logic Tensor Networks (LTNs) to express and test human-interpretable constraints on model representations and outputs.

The research is organized around three goals: (1) characterize representational phase transitions during training (with emphasis on grokking and late generalization) by tracking sparse features and their stability; (2) study how internal signals change under steering and constraint-based decoding, testing whether models exhibit detectable ``awareness'' of external steering mechanisms; and (3) detect distribution shifts and adversarial perturbations by identifying consistent activation/attention signatures that precede output degradation.
