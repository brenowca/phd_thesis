...
The two synthetic datasets introduced in this chapter enable the induction of grokking with minimal tuning of model parameters, avoiding an extensive search of the parameter space. These datasets enable the creation of a shift between training and test set distributions, allowing for user control over how much information each subclass carries, as detailed below. The datasets are available in the supplementary material accompanying this thesis.
...
As a concrete example, consider four classes with two subclasses each. When one subclass per class is strongly under-sampled, the model still receives weak relational signals from neighboring subclasses. This controlled imbalance is sufficient to induce delayed generalization in our setting, and motivates the full formal definition below.
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[t]{0.475\textwidth}
         ...
         \caption{A schematic of the equidistant dataset. Each subclass is equidistant from all others, creating a challenging scenario for generalization.}
    ...
    \begin{subfigure}[t]{0.475\textwidth}
         ...
         \caption{A schematic of the equivariant dataset. Subclasses are grouped by similarity into larger classes, providing a relational structure for the model to learn.}
